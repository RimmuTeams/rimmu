@extends('layouts.master')

@section('styles')
	@parent
	<!-- costume css here -->

   	
@stop

@section('scripts')
	@parent
	<!-- costume scripts here -->

   	
@stop

@section('content')
   <!-- Page header 
					<article class="page-header">
						<h1>Welcome, username!</h1>
					</article> -->
					<!-- /Page header -->

					<a href="#" class="btn btn-flat btn-danger" > Hot </a> 
					<a href="#" class="btn btn-flat"> Tranding</a> 
					<a href="#" class="btn btn-flat"> New</a> 
					<!-- Grid row -->
					<div class="row">
					
					<div class="span12">
						<form class="form-search">
							<div class="control-group">
								<div class="controls">
									<input class="span9" type="text" placeholder="Search Tournament">
								</div>
							</div>
						</form>
					</div>
					
					</div>
					<!-- /Grid row -->
					<a href="#" class="btn btn-flat" ><span class="awe-plus"></span> Create </a> 
					<a href="#" class="btn btn-flat"> My Quiz</a> 
					<!-- Grid row -->
					<div class="row">
					
					
					<!-- Data block -->
					<article class="span4 data-block">
						<div class="data-container">
							<header>
								<h2>Quiz Title</h2>
							</header>
							<section>
								<ul class="unstyled">
									<li><p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Phasellus quis erat dui, quis purus.</li>
									<li><a class="btn btn-info" href="#">Learn More</a></li>
								</ul>
							</section>
						</div>
					</article>
					<!-- /Data block -->
					
					<!-- Data block -->
					<article class="span4 data-block">
						<div class="data-container">
							<header>
								<h2>Quiz Title</h2>
							</header>
							<section>
								<ul class="unstyled">
									<li><p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Phasellus quis erat dui, quis purus.</li>
									<li><a class="btn btn-info" href="#">Learn More</a></li>
								</ul>
							</section>
						</div>
					</article>
					<!-- /Data block -->
					
					<!-- Data block -->
					<article class="span4 data-block">
						<div class="data-container">
							<header>
								<h2>Quiz Title</h2>
							</header>
							<section>
								<ul class="unstyled">
									<li><p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Phasellus quis erat dui, quis purus.</li>
									<li><a class="btn btn-info" href="#">Learn More</a></li>
								</ul>
							</section>
						</div>
					</article>
					<!-- /Data block -->
						
						
					</div>
					<!-- /Grid row -->
					
					<!-- Grid row -->
					<div class="row">
					
					<!-- Data block -->
					<article class="span4 data-block">
						<div class="data-container">
							<header>
								<h2>Quiz Title</h2>
							</header>
							<section>
								<ul class="unstyled">
									<li><p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Phasellus quis erat dui, quis purus.</li>
									<li><a class="btn btn-info" href="#">Learn More</a></li>
								</ul>
							</section>
						</div>
					</article>
					<!-- /Data block -->
					
					<!-- Data block -->
					<article class="span4 data-block">
						<div class="data-container">
							<header>
								<h2>Quiz Title</h2>
							</header>
							<section>
								<ul class="unstyled">
									<li><p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Phasellus quis erat dui, quis purus.</li>
									<li><a class="btn btn-info" href="#">Learn More</a></li>
								</ul>
							</section>
						</div>
					</article>
					<!-- /Data block -->
					
					<!-- Data block -->
					<article class="span4 data-block">
						<div class="data-container">
							<header>
								<h2>Quiz Title</h2>
							</header>
							<section>
								<ul class="unstyled">
									<li><p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Phasellus quis erat dui, quis purus.</li>
									<li><a class="btn btn-info" href="#">Learn More</a></li>
								</ul>
							</section>
						</div>
					</article>
					<!-- /Data block -->
						
						
					</div>
@stop


@section('other_script')
    @parent
    <!-- costume scripts_other here -->
   	
@stop